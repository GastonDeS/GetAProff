...
//                        .antMatchers(HttpMethod.POST, API_PREFIX+"/classroom/{classId}/{status}").access("@antMatcherVoter.canAccessClassroom(authentication, #classId)") // Only de student can rate, and idk if teacher can only one thing
                        ...
                        .antMatchers(HttpMethod.GET, API_PREFIX+"/subject-files/{fileId}").access("@antMatcherVoter.canAccessGetSubjectFile(authentication, #fileId)")
                        .antMatchers(HttpMethod.POST, API_PREFIX+"/subject-files/{fileId}").access("@antMatcherVoter.canAccessShareFile(authentication, #fileId)")
    ...
    public void configure(final WebSecurity web) throws Exception {
        web
                ...
                .antMatchers(HttpMethod.GET,API_PREFIX+"/favourites/{teacherId}")
                .antMatchers("/")
                .antMatchers("/*.js")
                .antMatchers("/*.css")
